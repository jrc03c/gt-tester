...
// node_modules/@jrc03c/base-web-component/dist/base-web-component.import.mjs
...
function stringify(x, indent2) {
  ...
  return helper52(decycle(x), indent2);
}
...
var BaseComponent = class extends HTMLElement {
  ...
  constructor(options) {
    super();
    options = options || {};
    sort2(set2(this.constructor.observedAttributes)).forEach((attr) => {
  ...
  getAttribute(attr) {
    const out3 = super.getAttribute(attr);
    try {
...
var ControlsComponent = class extends BaseComponent {
  ...
  constructor(options) {
    ...
    const lineAwesomeStylesheet = stylesheets.find(
      (sheet) => sheet.href.includes("line-awesome")
    );
    if (lineAwesomeStylesheet) {
      allCss.push(
        Array.from(lineAwesomeStylesheet.cssRules).map((rule) => rule.cssText).join("\n")
      );
    }
  ...
  mount() {
    const out3 = super.mount(...arguments);
